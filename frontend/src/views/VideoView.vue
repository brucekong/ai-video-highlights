...
import { Loader2, Sparkles, AlertCircle, FileText, Clock, Play, Send, MessageCircle, User as UserIcon, Bot, Map, Search, Share2, Copy, Scissors, Download, FileJson } from 'lucide-vue-next';
...
const exportAsMarkdown = () => {
  if (!takeaways.value.length) return;

  let md = `# ${videoTitle.value || '视频学习笔记'}\n\n`;
  md += `> 📺 视频来源: [点击播放](${videoUrl.value})\n`;
  md += `> 📅 分析日期: ${new Date().toLocaleDateString()}\n\n`;

  md += `## 🤖 AI 核心摘要\n\n`;
  takeaways.value.forEach((t, i) => {
  ...
  if (mindmapRaw.value) {
    ...
    const lines = mindmapRaw.value.split('\n');
  ...
  copyToClipboard(md, 'Markdown 笔记已准备好，去 Notion/Obsidian 粘贴吧！');
};
...
const generateEditingScript = () => {
  ...
  takeaways.value.forEach((t, i) => {
    ...
    const nextStart = takeaways.value[i+1]?.timestamp || (start + 20);
  ...
  script += `\n## 📝 旁白/叙文建议\n\n`;
  script += takeaways.value.map((t, i) => `【片段${i+1}】展现：${t.title}。建议文案：${t.summary}`).join('\n\n');

  copyToClipboard(script, '剪辑脚本与 FFMPEG 指令已生成！');
};
              ...
              <div class="sidebar-header">
                <div class="sidebar-title-area">
                   ...
                   <div class="sidebar-actions">
                    ...
                    <button class="btn-action-outline primary" title="导出 Markdown 笔记" @click="exportAsMarkdown" :disabled="!takeaways.length">
                      <Share2 :size="14" />
                      <span>导出</span>
                    </button>
                    <button class="btn-action-outline" title="生成剪辑脚本" @click="generateEditingScript" :disabled="!takeaways.length">
                      <Scissors :size="14" />
                      <span>剪辑</span>
                    </button>
